...
export type Emitting = {
    ...
    builtin: { usePopup: boolean } | { keepPopup: boolean }
}
...
export default class Bridge extends Emitter<Emitting> {
    ...
    private listener = (e: MessageEvent) => {
        ...
        const { method, params, id, result, error, session } = e.data as {
            [key: string]: unknown
        }
        console.info(
            `WalletConnector:${
                e.source === this._popup.window ? "popup" : "iframe"
            }`,
            e.data
        )
        if (id != null) {
        ...
        if (!is<Emitting["message"]>(emitting)) {
    ...
    postMessage(message: object, timeout?: number) {
        ...
        if (timeout) {
            setTimeout(
                () => this._promiseController[id].reject("timeout"),
                timeout
            )
        }
        return promise
    ...
    private openIframe() {
        ...
        const promise = new Promise(
            (resolve, reject) => (this._iframe = { resolve, reject })
        )
    ...
    private openPopup(force?: boolean) {
        ...
        const popupWindow = window.open(
            this._url.toString(),
            "_blank",
            "location,resizable,scrollbars,width=400,height=600"
        )
        const promise = new Promise(
            (resolve, reject) =>
                (this._popup = { window: popupWindow, resolve, reject })
        )
    ...
    completeRequest() {
        setTimeout(() => !this._pending.length && this.closePopup(), 100)
    }
    ...
    deliverMessage(message: any) {
        ...
        this._iframe.promise = this._iframe.promise
            ?.then(() =>
                this._iframe.window?.postMessage(fullMessage, this._url.origin)
            )
            .catch(() => {
                return
            })
        ...
        this._popup.promise = this._popup.promise
            ...
            .then(() =>
                this._popup.window?.postMessage(fullMessage, this._url.origin)
            )
            .catch(() => {
                return
            })
